...
初始化器先在目标目录旁创建随机命名的临时目录，再通过目录重命名完成提交。进程中断、写入失败或并发初始化失败后，父目录中可能保留以下内容：

- `.<name>.staging-*` 是尚未提交或提交失败的 staging 目录，可能只包含部分文件，也可能包含已写完但尚未成为目标目录的候选包。
- `.<name>.empty-backup-*` 是初始化器为保护原有空目标目录而生成的随机备份。它只表示提交过程捕获过一个空目录，不表示初始化已经成功。

这些残留不自动清理，也不应视为有效本体包。处理前应暂停对同一目标的初始化操作，并逐一核验目标目录和每个残留目录的实际内容：确认目标是否已经包含完整的六个包文件，检查残留中的 `manifest.yaml` 是否存在且是否与其余文件一致，同时结合进程、任务或锁的状态确认目录已无人使用。

确认无人使用后，优先把需要进一步检查的精确残留目录人工移动到独立隔离目录；只有在确认其内容不再需要、目标包状态明确且已保留必要审计证据后，才人工删除该精确目录。不要使用宽泛递归删除或 glob 匹配处理这类残留，以免误删当前包、其他并发 staging、备份目录或相邻私有数据。
