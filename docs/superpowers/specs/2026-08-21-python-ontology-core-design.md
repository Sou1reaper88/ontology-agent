...
- `docs/project-journal/YYYY-MM.md`：记录实施日期、任务背景、难点、排查过程、方案取舍、验证证据和结果。
- `docs/decisions/ADR-xxxx-*.md`：记录影响长期架构的重要决策、备选方案和后果。
- `docs/career/project-story.md`：将已验证的阶段成果提炼为简历表述和面试 STAR 素材。
...
2. 解决非简单缺陷、回归或性能问题。
3. 做出会影响后续模块的重要技术选型。
4. 解决值得复用的安全、兼容性或工程化问题。
5. 一个实施阶段结束，需要汇总已验证成果。

记录必须遵守以下约束：

- 只记录已发生的事实；未完成能力明确标记为计划，不写成成果。
- 量化结果引用测试输出、Git 提交或其他可复核证据，不虚构指标。
- 不记录真实表名、字段名、用户数据、密钥、内部地址和业务敏感口径。
- 工程日志保留技术细节，职业素材使用业务可理解、面试可讲述的语言。
